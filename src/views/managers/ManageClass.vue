  ...
  <div class="animated fadeIn">
    ...
    <div class="input-group mb-3">
      <input
        ...
        class="form-control"
        placeholder="新增類別名稱"
        aria-describedby="classBtn"
        v-model="className"
        required
      />
      <div class="input-group-append">
        <button class="btn btn-outline-secondary" type="button" id="classBtn" @click="newClass" style="background-color: white"><b>新增</b></button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    newClass() {
      ...
      if(this.className != "") {
        this.axios
          .post("http://127.0.0.1:8000/api/Admin/addCategory", {
          ...
          .catch(error => {
            if(error.response.data.errors.category != undefined)
            alert("重複輸入");
